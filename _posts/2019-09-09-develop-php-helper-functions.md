---
layout: post
title: Develop PHP helper functions
date: 2019-09-09 02:31 +0000
---


* [Implementing Namespaces in PHP 5.6](https://www.the-art-of-web.com/php/implementing-namespaces/)
* [Creating Your Own PHP Helpers in a Laravel Project](https://laravel-news.com/creating-helpers)
* [A Complete Guide to PHP Namespaces](https://www.thoughtfulcode.com/a-complete-guide-to-php-namespaces/)
* [FAQ: things you need to know about namespaces](https://www.php.net/manual/en/language.namespaces.faq.php#language.namespaces.faq.nofuncconstantuse)
* [Creating Your Own PHP Helper Functions In Laravel](https://tutsforweb.com/creating-helpers-laravel/)
* [fobiaweb/php-helper-lib](https://packagist.org/packages/fobiaweb/php-helper-lib)


---